...
export function validateTimeAndCursor(
  ...
  const validatedValue = showHours ? `${newH}${colon}${newM}${colon}${newS}` : `${newH}${colon}${newM}`;
...
export default class TimeFieldNoLimit extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    const _showHours = Boolean(props.showHours);
    const _defaultValue = _showHours ? DEFAULT_VALUE_FULL : DEFAULT_VALUE_SHORT;
    const _colon = props.colon && props.colon.length === 1 ? props.colon : DEFAULT_COLON;
    const [validatedTime] = validateTimeAndCursor(_showHours, this.props.value, _defaultValue, _colon);
  ...
  render(): ReactElement {
    ...
    const {onChange, style, showHours, input, inputRef, colon, ...props} = this.props; //eslint-disable-line no-unused-vars
    ...
    return (
      <input
        ...
        value={value}
        onChange={onChangeHandler}
        style={{width: showHours ? 54 : 35, ...style}}
      />
    );
  }
}